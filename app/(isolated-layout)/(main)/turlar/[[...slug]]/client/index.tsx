...
import PriceRangeSliderSection from "./price-range-slider";
import { TurCard } from "./tourcard";
import { toursConfig } from "@lib/ui-config/tours.config";

export default function TurlarClient(
  {
    kategori,
  }: {
  ...
  // Sayfalama
  const totalPages = Math.ceil(sortedTurlar.length / toursConfig.itemsPerPage);
  const startIndex = (currentPage - 1) * toursConfig.itemsPerPage;
  const endIndex = startIndex + toursConfig.itemsPerPage;
  ...
  const FilterContent = () => (
    <>
      ...
      <PriceRangeSliderSection priceRange={priceRange} setPriceRange={setPriceRange} />
